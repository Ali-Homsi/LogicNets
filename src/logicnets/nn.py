...
class SparseLinear(nn.Linear):
    ...
    def forward(self, input: Tensor) -> Tensor:
        # print(input.dim()) # 3
        # print(input.shape) #input shape torch.Size([1024, 128, 1])
        # a = self.weight*self.mask()
        # b = a.t()
        # print(b.dim()) # 2
        # print(b.shape) # torch.Size([128, 64])
        # RuntimeError: size mismatch, m1: [1024*128=131072 x 1], m2: [128 x 64] -> so b.shape has to be from the form [1 x whatever]

        # print("weights", self.weight)
        # print("weight shape", self.weight.shape)
        #
        # print("mask",self.mask.mask)
        # print("mask shape", self.mask.mask.shape)
        #
        # x = self.weight * self.mask
        # print("x =",x)
        # print("x shape =",x.shape)

        print(self.weight)
        print(self.mask)
        print("multiplied", self.weight*self.mask())
        input()
        return F.linear(input, self.weight*self.mask(), self.bias)


#ALI: I created this class
class SparseConv1d(nn.Conv1d):
    def __init__(self, in_channels: int, out_channels: int,mask: nn.Module, bias: bool = True ) -> None:
        #Kernel size = 5 optional
        super(SparseConv1d, self).__init__(in_channels=in_channels, out_channels=out_channels, kernel_size=5, stride=1, padding=0,dilation=1,groups=1,bias=bias, padding_mode='zeros' )
        self.mask = mask

    def forward(self, input: Tensor) -> Tensor:
        # print("forwarding through conv layer")

        # print("weights", self.weight)
        # print("weight shape", self.weight.shape) # torch.Size([8, 1, 5])
        #
        # print("mask",self.mask.mask)
        # print("mask shape", self.mask.mask.shape) # torch.Size([8, 1])
        # x = self.weight * self.mask
        # print("x =",x)
        # print("x shape =",x.shape)


        # return F.conv1d(input, self.weight * self.mask(), self.bias,stride=1,padding="valid",dilation=1,groups=1) #from stride onward: the default values for F.conv1d
        return F.conv1d(input, self.weight * self.mask() , self.bias, stride=1)

#ALI: I created this class
class SparseConv1dNeq(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, input_quant, output_quant, sparse_linear_kws={}, apply_input_quant=True, apply_output_quant=True) -> None:
        super(SparseConv1dNeq, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.input_quant = input_quant
        self.conv = SparseConv1d(in_channels= in_channels,out_channels = out_channels , **sparse_linear_kws)
        ...
        self.isConv = True #ALI: True if the module is a conv module. False if it's linear.
...
class SparseLinearNeq(nn.Module):
    def __init__(self, in_features: int, out_features: int, input_quant, output_quant, sparse_linear_kws={}, apply_input_quant=True, apply_output_quant=True) -> None:
        ...
        self.isConv = False  # ALI: True if the module is a conv module. False if it's linear.
...
#ALI: I created this class to add 3d random fixed sparsity. Use this mask for conv1d layers
class RandomFixedSparsityMask3D(nn.Module):
    def __init__(self, in_features: int, out_features: int, fan_in: int, kernel_size = 5) -> None:
        super(RandomFixedSparsityMask3D, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.fan_in = fan_in
        self.kernel_size = kernel_size
        self.mask = Parameter(torch.Tensor(out_features, in_features,kernel_size), requires_grad=False)
        # print(self.mask)
        self.reset_parameters()
        # print(self.mask)
