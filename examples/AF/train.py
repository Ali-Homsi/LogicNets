...
from dataset import AtrialFibrillationDataset
from models import AtrialFibrillationNeqModel , AtrialFibrillationConv1dNeqModel
...
def train(model, datasets, train_cfg, options):
    ...
    for epoch in range(0, num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # print("train target_label = ", target_label)
            # print("target_label shape= ", target_label.shape)


            # ALI currently correct in this batch
            curCorrect = pred.eq(target_label).long().sum()
            # print("train pred.eq(target_label) = ", pred.eq(target_label))
            # print("train pred.eq(target_label).long() = ", pred.eq(target_label).long())
            # print("train curCorrect = ", curCorrect)
            # input()

            curAcc = 100.0*curCorrect / len(data) # logicnets defined this variable but didn't use it
            # print("curAcc = ", curAcc)
...
def test(model, dataset_loader, cuda):
    # model.eval() #when using this in the AF example, if I pass the data through the model, the output is always constant: test output = tensor([[0.3269, 0.3269])
    correct = 0
    accLoss = 0.0

    with torch.no_grad():     # ALI: I will use "with torch.no_grad()" for now instead of model.eval()
        for batch_idx, (data, target) in enumerate(dataset_loader):
            if cuda:
                data, target = data.cuda(), target.cuda()
            #ALI same as in the training loop, flatten the data
            # data = data.view(-1, 5250) # ONLY FOR LINEAR MODEL
            output = model(data)
            # print("test output =", output) # ALI: why is this always giving constant value tensor([[0.3269, 0.3269]-repeated-) after calling model.eval()
            pred = output.detach().max(1, keepdim=True)[1] #ALI: when model.eval() is called, this has only ones inside, since there is no max of [0.3269, 0.3269]
            target_label = torch.max(target.detach(), 1, keepdim=True)[1]
            curCorrect = pred.eq(target_label).long().sum()
...
if __name__ == "__main__":
    ...
    # Fetch the datasets
    dataset = {}
    dataset['train'] = AtrialFibrillationDataset(split="train")
    dataset['valid'] = AtrialFibrillationDataset(split="valid")
    dataset['test'] = AtrialFibrillationDataset(split="test")
    ...
    model_cfg['input_length'] = 1 # 1 channel with 5250 input if the dataset windowsize is 42 Sec, 1250 if the windowsize of the dataset is 10 sec.
    model_cfg['output_length'] = 2

    model = AtrialFibrillationConv1dNeqModel(model_cfg)
    # model = AtrialFibrillationNeqModel(model_cfg)
